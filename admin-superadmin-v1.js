...
(function(){
  ...
  function shell(){
    ...
    root.innerHTML='<div class="page-head"><div><h1>👑 Area Superadmin</h1><p>Controllo accessi, registro modifiche, backup e ripristino.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="btn" id="openSuperadminHelp">❓ Help</button><button type="button" class="btn" id="closeSuperadmin">← Torna al pannello</button></div></div>'+
  ...
  async function openPanel(){
    ...
    $('superadminCapabilities').innerHTML='<div style="display:grid;gap:14px"><div><h3 style="margin:0 0 6px">✅ Può fare</h3><ul style="margin:0;padding-left:20px"><li>Gestire tutte le funzioni disponibili all’Admin.</li><li>Eliminare definitivamente tornei e iscrizioni.</li><li>Archiviare e riaprire tornei.</li><li>Consultare audit e backup.</li><li>Ripristinare dati da backup disponibili.</li><li>Gestire i ruoli utenti a livello database, inclusa la protezione contro l’auto-promozione degli Admin.</li></ul></div><div><h3 style="margin:0 0 6px">↩️ Copertura ripristino</h3><table style="width:100%;border-collapse:collapse"><tr><th style="text-align:left;padding:7px;border-bottom:1px solid #ddd">Elemento</th><th style="text-align:left;padding:7px;border-bottom:1px solid #ddd">Backup</th><th style="text-align:left;padding:7px;border-bottom:1px solid #ddd">Ripristino</th></tr><tr><td style="padding:7px">Torneo + configurazione/regole</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr><tr><td style="padding:7px">Iscrizioni</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr><tr><td style="padding:7px">Profili/ruoli</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr><tr><td style="padding:7px">News</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr><tr><td style="padding:7px">Sponsor</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr><tr><td style="padding:7px">Mercatino</td><td style="padding:7px"><b>SI</b></td><td style="padding:7px"><b>SI</b></td></tr></table></div><div style="padding:10px 12px;border-left:4px solid #198754;background:#eefaf2"><b>Come funziona:</b> per i dati amministrativi il sistema conserva lo snapshot della singola riga quando viene inserita, modificata o cancellata. Il Superadmin può quindi riportare quella riga allo stato salvato. Non è un’immagine completa dell’intero database in un unico backup.</div><div style="padding:10px 12px;border-left:4px solid #9a6700;background:#fff8e6"><b>Importante:</b> il ripristino è possibile solo se esiste il relativo snapshot. Un dato creato e mai passato da un backup non può essere ricostruito.</div></div>';
  ...
  async function loadBackups(){
    ...
    const rows=r.data||[];
    box.innerHTML=rows.length?'<div class="list">'+rows.map(x=>{const nome=x.snapshot?.nome||('Torneo #'+x.torneo_id);return '<div class="list-item"><div class="list-item-main"><div><strong>'+esc(nome)+'</strong><small>Backup #'+esc(x.id)+' · '+esc(x.reason||'')+' · '+esc(x.created_by_email||'-')+' · '+esc(dt(x.created_at))+'</small></div><button type="button" class="btn small danger" data-restore-backup="'+esc(x.id)+'">↩ Ripristina</button></div></div>';}).join('')+'</div>':'<div class="empty">Nessun backup disponibile.</div>';
  ...
  const GITHUB_REPO='torneipadel/torneipadelnew.github.io';
  ...
  async function loadCodeBackups(){
    ...
    const registry={};(reg.data||[]).forEach(x=>{registry[x.file_path+'@'+x.commit_sha]=x});
    const results=await Promise.all(CODE_FILES.map(async file=>{try{const r=await fetch('https://api.github.com/repos/'+GITHUB_REPO+'/commits?path='+encodeURIComponent(file)+'&per_page=12',{headers:{Accept:'application/vnd.github+json'}});if(!r.ok)throw new Error('GitHub HTTP '+r.status);const data=await r.json();return {file,commits:data};}catch(e){return {file,commits:[],error:e.message}}}));
    let html='<div style="margin-bottom:16px;padding:14px;border:1px solid #198754;border-radius:10px;background:#eefaf2"><div style="font-weight:800;font-size:16px">💾 Backup completo progetto</div><div style="margin-top:6px">Crea in qualsiasi momento un nuovo snapshot completo dell’intero repository, senza modificare <code>main</code>.</div><div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="btn small" id="createCompleteProjectBackup">💾 Esegui backup completo adesso</button><button type="button" class="btn small" id="refreshCompleteProjectBackups">↻ Aggiorna backup</button></div><div id="completeProjectBackupStatus" style="margin-top:10px"></div><div id="completeProjectBackupHistory" style="margin-top:10px"></div></div><div style="display:grid;gap:16px">';
    results.forEach(group=>{html+='<div style="border:1px solid #ddd;border-radius:10px;padding:12px"><h3 style="margin:0 0 10px">'+esc(group.file)+'</h3>';if(group.error){html+='<div class="empty">Errore: '+esc(group.error)+'</div></div>';return}if(!group.commits.length){html+='<div class="empty">Nessuna versione trovata.</div></div>';return}html+='<div style="display:grid;gap:8px">';group.commits.forEach(cm=>{const sha=cm.sha;const meta=registry[group.file+'@'+sha];const status=meta?.status||'TEST';const badge=status==='STABILE'?'🟢 STABILE':status==='PROBLEMATICA'?'🔴 PROBLEMATICA':'🟡 TEST';const msg=cm.commit?.message?.split('\n')[0]||'';const date=cm.commit?.author?.date||cm.committer?.date;html+='<div style="padding:10px;border:1px solid #eee;border-radius:8px"><div style="display:flex;gap:10px;justify-content:space-between;align-items:flex-start;flex-wrap:wrap"><div><strong>'+badge+' · '+esc(sha.slice(0,10))+'</strong><small style="display:block">'+esc(dt(date))+' · '+esc(msg)+'</small></div><div style="display:flex;gap:6px;flex-wrap:wrap"><a class="btn small" href="https://github.com/'+GITHUB_REPO+'/commit/'+sha+'" target="_blank" rel="noopener">Apri</a><a class="btn small" href="https://github.com/'+GITHUB_REPO+'/compare/'+sha+'...main" target="_blank" rel="noopener">Confronta</a><button type="button" class="btn small" data-code-status="STABILE" data-code-file="'+esc(group.file)+'" data-code-sha="'+sha+'">⭐ Stabile</button><button type="button" class="btn small" data-code-status="TEST" data-code-file="'+esc(group.file)+'" data-code-sha="'+sha+'">🟡 Test</button><button type="button" class="btn small" data-code-status="PROBLEMATICA" data-code-file="'+esc(group.file)+'" data-code-sha="'+sha+'">🔴 Problematica</button><button type="button" class="btn small danger" data-code-restore="'+sha+'" data-code-file="'+esc(group.file)+'">↩ Ripristina</button></div></div></div>';});html+='</div></div>';});
    ...
    box.innerHTML=html;$('createCompleteProjectBackup')?.addEventListener('click',createCompleteProjectBackup);$('refreshCompleteProjectBackups')?.addEventListener('click',loadCompleteProjectBackups);await loadCompleteProjectBackups();box.querySelectorAll('[data-code-status]').forEach(b=>b.onclick=()=>setCodeStatus(b.dataset.codeFile,b.dataset.codeSha,b.dataset.codeStatus));box.querySelectorAll('[data-code-restore]').forEach(b=>b.onclick=()=>requestCodeRestore(b.dataset.codeFile,b.dataset.codeRestore));
  }

  async function loadCompleteProjectBackups(){
    const box=$('completeProjectBackupHistory');if(!box)return;box.innerHTML='<div class="empty">Lettura backup completi...</div>';
    try{const c=client();if(!c?.functions?.invoke)throw new Error('Servizio backup non disponibile');const r=await c.functions.invoke(COMPLETE_PROJECT_BACKUP_FUNCTION,{method:'GET'});if(r.error)throw r.error;const rows=(r.data?.backups||[]).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));box.innerHTML=rows.length?'<div style="display:grid;gap:6px">'+rows.map(b=>'<div style="padding:9px;border:1px solid #eee;border-radius:8px"><div><b>'+esc(b.branch_name)+'</b> · '+esc(dt(b.created_at))+'</div><small>Commit: <code>'+esc(String(b.commit_sha||'').slice(0,10))+'</code> · '+esc(b.created_by_email||'-')+'</small><div style="margin-top:5px"><a href="https://github.com/'+GITHUB_REPO+'/tree/'+encodeURIComponent(b.branch_name)+'" target="_blank" rel="noopener">Apri backup</a> · <a href="https://github.com/'+GITHUB_REPO+'/commit/'+encodeURIComponent(b.commit_sha)+'" target="_blank" rel="noopener">Apri commit</a></div></div>').join('')+'</div>':'<div class="empty">Nessun backup completo trovato.</div>';}catch(e){box.innerHTML='<div class="empty">Impossibile leggere i backup completi: '+esc(e.message||e)+'</div>'}
  }

  async function createCompleteProjectBackup(){
    if(!isSuper())return;
    const status=$('completeProjectBackupStatus');const button=$('createCompleteProjectBackup');if(button)button.disabled=true;if(status)status.innerHTML='<span class="notice">Creazione backup completo in corso...</span>';
    try{const c=client();if(!c?.functions?.invoke)throw new Error('Servizio backup non disponibile');const r=await c.functions.invoke(COMPLETE_PROJECT_BACKUP_FUNCTION,{method:'POST',body:{}});if(r.error)throw r.error;if(!r.data?.ok)throw new Error(r.data?.error||'Backup non creato');const b=r.data.backup||{};if(status)status.innerHTML='<span class="notice">✅ Backup completo creato: <code>'+esc(b.branch_name||'-')+'</code> · commit <code>'+esc(String(b.commit_sha||'').slice(0,10))+'</code></span>';await loadCompleteProjectBackups();}catch(e){let detail='';try{if(e?.context?.json){const body=await e.context.json();detail=body?.error||body?.message||'';}}catch(_){}const msg=detail||e?.message||String(e);if(status)status.innerHTML='<span class="notice" style="color:#b42318">❌ Backup non creato: '+esc(msg)+'</span>';console.error('Backup completo progetto:',e,detail)}finally{if(button)button.disabled=false}
  }
  ...
  async function restoreData(id){
    ...
    await loadAudit();await loadDataBackups();
  }
  ...
  async function restore(id){
    ...
    await window.caricaTorneiSupabase?.();await loadAudit();await loadBackups();
  }
  ...
  function boot(){roleReady();const app=$('appContent');if(app)obs.observe(app,{childList:true,subtree:true});}
